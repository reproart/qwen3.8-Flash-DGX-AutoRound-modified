...
class MmapPleTable:
    ...
    def gather(self, ids: np.ndarray, already_unique: bool = False) -> np.ndarray:
        """ids: int64 [N] global row ids -> uint8 [N, row_bytes] (a fresh array).

        already_unique=True skips the internal dedup+sort pass — for callers
        that ran np.unique themselves and can use its inverse for the GPU-side
        expansion (the placeholder embedding's forward does exactly that).
        """
        import time as _time

        t0 = _time.perf_counter()
        try:
            return self._gather(ids, already_unique=already_unique)
        finally:
            _stats_add(gather_ms=(_time.perf_counter() - t0) * 1e3,
                       rows=int(np.asarray(ids).size),
                       bytes=int(np.asarray(ids).size) * self.row_bytes)

    def _gather(self, ids: np.ndarray, already_unique: bool = False) -> np.ndarray:
        ...
        # the caller deduplicated anyway (it still needs the inverse index).
        if already_unique:
    ...
    def prewarm(self) -> None:
        ...
        block = 64 << 20
        buf = bytearray(block)
        mv = memoryview(buf)  # reused: no per-iteration 64 MB allocations
        for path, mm in zip(self.paths, self.mm):
            ...
            with open(path, "rb", buffering=0) as f:
                pos = start
                while pos < end:
                    n = f.readinto(mv[: min(block, end - pos)])  # noqa: F841
                    if not n:
                        break
                    pos += n
...
class PrefetchingMmapTable:
    ...
    def gather(self, ids: np.ndarray, already_unique: bool = False) -> np.ndarray:  # sync fallback path
        return self.inner.gather(ids, already_unique=already_unique)
...
class _MmapNgramEmbedding(nn.Module):
    ...
    def _pinned_idx(self, n: int) -> torch.Tensor | None:
        """Pinned int64 buffer for the inverse-index H2D copy — from pageable
        memory torch's non_blocking copy is effectively synchronous."""
        buf = getattr(self, "_pinned_inv", None)
    ...
    def forward(self, ids: torch.Tensor) -> torch.Tensor:
        ...
        # uniq is already sorted+unique — skip gather's own dedup pass.
        rows = table.gather(uniq, already_unique=True)  # uint8 [U, row_bytes]
        u = rows.shape[0]
        ...
        inv_buf = self._pinned_idx(inverse.size) if ids.device.type == "cuda" else None
        if inv_buf is not None:
            inv_buf[: inverse.size].numpy()[:] = inverse.reshape(-1)
            inv = inv_buf[: inverse.size].to(ids.device, non_blocking=True)
        else:
            inv = torch.from_numpy(inverse.reshape(-1)).to(ids.device)
...
def _itemsize(dtype_str: str) -> int:
    try:
        ...
        raise ValueError(
            f"unsupported dtype {dtype_str!r} in the PLE table shards "
            "(expected F8_E4M3/F8_E5M2/BF16/F16)"
        ) from None


def _read_scale(entry: tuple) -> torch.Tensor:
    path, offset, nbytes, dtype_str = entry
    need = {"F32": 4, "BF16": 2, "F16": 2}
    if dtype_str not in need:
        raise ValueError(f"unsupported weight_scale dtype {dtype_str!r} ({path})")
    if nbytes < need[dtype_str]:
        raise ValueError(
            f"weight_scale in {path}: {nbytes} bytes, need >= {need[dtype_str]}"
        )
    with open(path, "rb") as f:
        f.seek(offset)
        raw = f.read(nbytes)
...
# Lifetime twin of _STATS (never reset): read by the vllm_custom_metrics sidecar
# so Prometheus counters survive the log-window resets.
_STATS_LIFE = {"calls": 0, "op_ms": 0.0, "gather_ms": 0.0, "rows": 0, "bytes": 0}
_STATS_LAST = [0.0]
_STATS_SEC = _env_int("VLLM_PLE_MMAP_STATS_SEC", 30)


def _stats_add(**kw) -> None:
    """Accumulate into the resettable window stats and the lifetime counters."""
    for k, v in kw.items():
        _STATS[k] = _STATS.get(k, 0) + v
        _STATS_LIFE[k] = _STATS_LIFE.get(k, 0) + v
    _metrics_start()
...
def _metrics_start() -> None:
    """Best-effort lazy start of the engine-side metrics sidecar (once).

    Started on the first stat push — i.e. from the process that actually runs
    the engine — rather than at import, so a frontend process that merely
    imports the model module cannot grab the metrics port and serve zeros.
    """
    global _METRICS_MOD
    if _METRICS_MOD is None:
        try:
            import vllm_custom_metrics as _mod
            _mod.start()
            _METRICS_MOD = _mod
        except Exception:
            _METRICS_MOD = False
...
def _lookup_impl(
    ...
    _stats_add(calls=1, op_ms=(_time.perf_counter() - t0) * 1e3)
    _stats_log()
