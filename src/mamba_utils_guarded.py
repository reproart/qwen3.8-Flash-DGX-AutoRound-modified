...
class MambaSpecDecodeGPUContext:
    ...
    def run_fused_precopy(
        ...
        if self.guard_step % 512 == 0:
            ...
            # engine-side /metrics sidecar (vllm:mamba_state_copy_guard_total);
            # best-effort, never let telemetry touch the hot path
            try:
                import vllm_custom_metrics
                vllm_custom_metrics.set_guard_hits(hits)
            except Exception:
                pass
