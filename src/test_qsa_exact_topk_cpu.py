...
Inside the image it tests the helpers as installed in qsa.py:
    docker run --rm -v "$PWD/src:/t" -w /t --entrypoint python3 qwen38-flash-dgx test_qsa_exact_topk_cpu.py
Anywhere else (CI, a laptop with torch) it tests the same code taken from the
patch script itself.
"""
import os
import re
...
QSA = f"{SP}/vllm/models/qwen3_8_flash_next/nvidia/ops/qsa.py"
MARK = "# --- GX10: QSA top-k variants"
if os.path.exists(QSA) and MARK in open(QSA).read():
    src = open(QSA).read()
    tail, where = src[src.index(MARK):], QSA
else:
    patch = open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "patch_qsa_exact_topk.py")).read()
    tail, where = re.search(r"src \+= '''(.*?)'''", patch, re.S).group(1), "patch_qsa_exact_topk.py"
...
print(f"exact top-k: OK (sets match torch.topk over visible columns, deterministic) [{where}]")
